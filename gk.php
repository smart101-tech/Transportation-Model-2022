...
<body>
    ...
    <script>
     ...
     <div class="container">
	<h1 class="text-info" style="text-align:center; padding-top:5px;padding-buttom:-10px" ><b>TRANSPORTATION MODEL - GK DASHBOARD</b></h1>
...
//	}
   ...
   if ((($lat<=9.537199) AND ($lat>=$pb_lat )) AND (($lng<=6.455822) AND ($lng>=$pb_long ))){
...
//	}
	...
	if ($edt==$count){
			$edt= date("h:m:sa");
			$sql = "UPDATE tbl_dept2 SET time='$edt' WHERE id='$id'";
			...
			$result = mysqli_query($db,"SELECT * FROM tbl_dept2 WHERE id='$id'") or die ("error" . mysqli_error($conn));
            if($result){				
        	     while($row = mysqli_fetch_assoc($result)){
			     $edt =$row['time'];
			    }   
